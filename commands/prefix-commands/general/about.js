...
module.exports = {
    ...
    execute: async (message) => {
        ...
        const botCreationDate = `<t:${Math.floor(message.client.user.createdTimestamp / 1000)}:R>`;
        const botUptime = `<t:${Math.floor((Date.now() - process.uptime() * 1000) / 1000)}:R>`;
        const totalServers = message.client.guilds.cache.size;
        const totalPrefixCommands = message.client.commands.size;
        const totalSlashCommands = message.client.slashCommands.size;

        const embed = new EmbedBuilder()
            .setColor(0x0099ff)
            .setTitle('ℹ️ Thông tin về bot')
            .addFields(
                { name: '🤖 Tên Bot', value: `${message.client.user.username}`, inline: true },
                { name: '🏷️ Tag Bot', value: `<@${message.client.user.id}>`, inline: true },
                { name: '🆔 ID Bot', value: `${message.client.user.id}`, inline: true },
